	...
	if (progress_bar && player_info.ti) {
		...
		if (duration && duration >= player_info.pos) {
			if (progress_bar == PROGRESS_BAR_LINE) {
				/* Fill blank space with progress bar line */
				int bar_start = len.llen + len.mlen;
				int bar_len = win_w - len.rlen - bar_start;
				if (bar_len >= 5) {
					bar_len = bar_len * player_info.pos / duration;
					for(int x = bar_start; x < bar_start + bar_len; ++x)
						(void) mvaddch(LINES - 2, x, ACS_HLINE);
				}
			...
			} else { // PROGRESS_BAR_COLOR_MARKER
				/* Redraw a few cols in alt color to mark the current position */
				int marker_len = min_u(6, win_w);
				int x = (win_w - marker_len) * player_info.pos / duration;

				int skip = x;
				int buf_index = u_skip_chars(print_buffer.buffer, &skip, false);

				int end_offset = u_skip_chars(print_buffer.buffer + buf_index, &marker_len, true);
