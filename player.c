 ...
 * buffer-fill changed
 */
static void producer_buffer_fill_update(void)
{
	int fill;
 ...
 */
static void consumer_position_update(void)
{
...
static void producer_status_update(enum producer_status status)
{
	producer_status =  status;
	pthread_cond_broadcast(&producer_playing);
}

static void producer_play(void)
{
	if (producer_status == PS_UNLOADED) {
		...
		if (get_next(&ti) == 0) {
			...
			if (rc) {
				...
				ip_setup(ip);
				producer_status_update(PS_PLAYING);
				file_changed(ti);
			}
		}
...
static void producer_pause(void)
{
	if (producer_status == PS_PLAYING) {
		producer_status_update(PS_PAUSED);
	} else if (producer_status == PS_PAUSED) {
		producer_status_update(PS_PLAYING);
	}
}

static void producer_set_file(struct track_info *ti)
{
	producer_unload();
	ip = ip_new(ti->filename);
	producer_status_update(PS_STOPPED);
...
static void consumer_status_update(enum consumer_status status)
{
	consumer_status = status;
	pthread_cond_broadcast(&consumer_playing);
}

static void consumer_play(void)
{
...
static void consumer_drain_and_stop(void)
{
	if (consumer_status == CS_PLAYING || consumer_status == CS_PAUSED) {
		op_close();
...
static void consumer_pause(void)
{
	if (consumer_status == CS_PLAYING) {
		op_pause();
		consumer_status_update(CS_PAUSED);
	} else if (consumer_status == CS_PAUSED) {
		op_unpause();
		consumer_status_update(CS_PLAYING);
	}
...
static void consumer_handle_eof(void)
{
	struct track_info *ti;

	if (ip_is_remote(ip)) {
		producer_stop();
		consumer_drain_and_stop();
