...
#define DB(name) { #name, get_ ## name, set_ ## name, toggle_ ## name, OPT_BOOL  },
...
static const struct {
	...
	DB(resume)
	DB(show_hidden)
	DB(auto_expand_albums_follow)
	DB(auto_expand_albums_search)
	DB(auto_expand_albums_selcur)
	DB(show_all_tracks)
	DB(show_current_bitrate)
	DB(show_playback_position)
	DB(show_remaining_time)
	DB(set_term_title)
	DB(shuffle)
	DB(follow)
	DB(softvol)
